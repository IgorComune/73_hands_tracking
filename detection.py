...
def coord_hands(img, lado_invertido = False):
...
while True:
    ...
    img = cv2.flip(img, 1)
    img, todas_maos = coord_hands(img)
    ...
    if len(todas_maos) == 1:
        ...
        if todas_maos[0]['lado'] == 'Left':
            ...
            cv2.putText(img, f'Distância câmera: {indicador_z}', (850, 50), cv2.FONT_HERSHEY_COMPLEX, 1, BRANCO, 2)
            ...
            cv2.circle(img, (indicador_x, indicador_y), 7, VERMELHO, 3)
    ...
    if len(todas_maos) == 2:
        ...
        print(img.shape)
        print(img_quadro.shape)
        
        # img_quadro = cv2.resize(img_quadro, (img.shape[1], img.shape[0]))
